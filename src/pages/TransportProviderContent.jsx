...
import { 
  Truck, MapPin, ShieldCheck, Star, 
  CheckCircle, Plus, Search, ChevronRight, 
  X, Edit3, Trash2, ExternalLink, 
  Check, MessageSquare, Info, Eye
} from 'lucide-react';
      ...
      <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 border-b border-slate-100 pb-4 sm:pb-5">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">
            ...
            {myService && !isEditingService && (
              <>
                <button
                  ...
                  className="bg-stone-100 hover:bg-stone-200 text-slate-800 text-xs font-black px-4 py-2.5 rounded-xl transition flex items-center justify-center gap-2 cursor-pointer shadow-sm w-full sm:w-auto"
                >
                  <Edit3 size={14} /> Edit Service Details
                </button>
                <button
                  onClick={() => navigate('/pet-transport')}
                  className="bg-[#0F2E23] hover:bg-[#164E3D] text-[#D4AF37] hover:text-white text-xs font-black px-4 py-2.5 rounded-xl transition flex items-center justify-center gap-2 cursor-pointer shadow-sm w-full sm:w-auto"
                >
        ...
        {myService && !isEditingService ? (
          <div className="space-y-4 sm:space-y-6">
            ...
            {/* Status Alert Banner */}
            <div className="bg-gradient-to-r from-emerald-50 via-emerald-100/60 to-emerald-50 border border-emerald-200 rounded-2xl p-4 sm:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 sm:gap-4">
              <div className="flex items-start sm:items-center gap-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-emerald-500 text-white flex items-center justify-center shadow-md shrink-0">
                  <CheckCircle size={20} />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="text-xs sm:text-sm font-black text-emerald-950">Active & Published on Pet Transport Directory</h4>
              ...
              <div className="flex items-center gap-2 shrink-0 self-start md:self-auto">
                <span className="text-[10px] sm:text-[11px] font-black text-emerald-900 bg-white/80 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl border border-emerald-200">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white rounded-2xl sm:rounded-3xl border-2 border-emerald-500/30 p-4 sm:p-6 shadow-lg hover:shadow-xl transition flex flex-col md:flex-row gap-4 sm:gap-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-[#0F2E23] text-[#D4AF37] text-[9px] sm:text-[10px] font-black px-3 sm:px-4 py-1 rounded-bl-xl shadow flex items-center gap-1">
                  <ShieldCheck size={12} /> Your Published Service
                </div>

                <div className="w-full md:w-56 shrink-0 space-y-3">
                  <div className="aspect-[16/10] sm:aspect-[4/3] rounded-2xl overflow-hidden relative shadow-md border border-stone-100">
                    <img
                      src={myService.image}
                      alt={myService.name}
                ...
                <div className="flex-1 flex flex-col justify-between space-y-4">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        <h3 className="text-lg sm:text-xl font-serif font-bold text-[#0F2E23]">
                          {myService.name}
                        </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-y-1 gap-x-2 sm:gap-x-3 text-xs text-gray-600">
                  ...
                  <div className="pt-3 border-t border-stone-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                    <div>
                      <div className="text-[10px] uppercase font-bold text-gray-400">Starting Rates</div>
                      <div className="flex items-baseline gap-1.5">
                        <span className="text-lg sm:text-xl font-serif font-bold text-[#0F2E23]">₹{myService.pricePerKm}/km</span>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                      <button
                        onClick={() => setIsEditingService(true)}
                        className="bg-stone-100 hover:bg-stone-200 text-slate-800 text-xs font-bold px-4 py-2.5 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 w-full sm:w-auto"
                      >
                        <Edit3 size={13} />
                      ...
                      <button
                        onClick={() => navigate('/pet-transport')}
                        className="bg-[#0F2E23] hover:bg-[#164E3D] text-[#D4AF37] hover:text-white text-xs font-bold px-4 sm:px-5 py-2.5 rounded-xl transition shadow-sm cursor-pointer flex items-center justify-center gap-1.5 w-full sm:w-auto"
                      >
          ...
          <form onSubmit={handleSaveService} className="space-y-6 sm:space-y-8 bg-slate-50/50 p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-slate-200">
            
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-4 gap-2">
              <div>
                <h3 className="text-base sm:text-lg font-sans font-black text-[#0F2E23]">
              ...
              {myService && (
                <button
                  ...
                  className="text-xs font-bold text-slate-500 hover:text-slate-800 flex items-center gap-1 self-end sm:self-auto"
                >
                  <X size={14} /> Cancel
                </button>
              )}
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {['Road Transport', 'Air Transport', 'Rail Transport', 'Ship Transport'].map((m) => {
                  ...
                  return (
                    <label
                      key={m}
                      className={`flex items-center gap-2 px-3.5 sm:px-4 py-2 rounded-xl border text-xs font-bold cursor-pointer transition ${
            ...
            {/* Certifications */}
            <div className="flex items-center gap-3 bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200">
              <input
                type="checkbox"
                id="iataCert"
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2.5 sm:gap-3 pt-4 border-t border-slate-200">
              {myService && (
                <button
                  type="button"
                  onClick={() => setIsEditingService(false)}
                  className="px-5 py-2.5 rounded-xl border border-slate-300 text-xs font-black text-slate-600 hover:bg-slate-100 transition cursor-pointer text-center"
                >
                  Cancel
                </button>
              )}
              <button
                type="submit"
                className="bg-[#0F2E23] hover:bg-[#164E3D] text-[#D4AF37] hover:text-white px-6 sm:px-8 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition shadow-md cursor-pointer flex items-center justify-center gap-2"
              >
                <Check size={16} />
  ...
  if (activeTab === 'appointments') {
    return (
      <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 border-b border-slate-100 pb-4 sm:pb-5">
          <div>
            <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">Transport Bookings & Active Trips</h2>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-1 sm:gap-2 bg-slate-100 p-1 rounded-xl overflow-x-auto no-scrollbar">
            {['All', 'Confirmed', 'In Transit', 'Completed'].map((f) => (
              <button
                key={f}
                onClick={() => setBookingFilter(f)}
                className={`px-3 sm:px-4 py-1.5 rounded-lg text-xs font-black transition shrink-0 ${
          ...
          <div className="relative w-full sm:w-64">
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 text-xs rounded-xl pl-8 pr-3 py-2.5 text-slate-800 focus:outline-none focus:border-[#0F2E23]"
            />
        ...
        <div className="space-y-3.5 sm:space-y-4">
          {filteredBookings.length === 0 ? (
            <div className="bg-slate-50 rounded-2xl p-4 sm:p-8 lg:p-10 text-center border border-slate-100">
            ...
            filteredBookings.map((b) => (
              <div
                key={b.id}
                className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm hover:shadow-md transition space-y-3 sm:space-y-4"
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2.5">
                  ...
                  <div className="flex items-center justify-between sm:justify-start gap-2 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                    <select
                      value={b.status}
                      onChange={(e) => handleUpdateBookingStatus(b.id, e.target.value)}
                      className="text-xs font-bold bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 focus:outline-none"
                    >
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-[#0F2E23] hover:underline px-2 py-1 cursor-pointer"
                    >
                      Trip Sheet
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 bg-slate-50/70 p-3.5 sm:p-4 rounded-xl text-xs">
                  <div>
  ...
  if (activeTab === 'vehicles') {
    return (
      <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 border-b border-slate-100 pb-4 sm:pb-5">
          <div>
            <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">Fleet Vehicles & Pricing Rates</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
          {vehicles.map((v) => (
            <div
              key={v.id}
              className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm hover:shadow-md transition space-y-3 sm:space-y-4 relative"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h4 className="font-sans font-black text-[#0F2E23] text-sm sm:text-base">{v.name}</h4>
              ...
              <div className="space-y-1.5 text-xs text-slate-600 bg-slate-50 p-3 sm:p-3.5 rounded-xl">
                <div className="flex justify-between">
    ...
    return (
      <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300">
        ...
        <div className="border-b border-slate-100 pb-4 sm:pb-5">
          <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">Client Relocation Inquiries</h2>
          ...
          {inquiries.length === 0 ? (
            ...
            inquiries.map((inq) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-50/70 p-3.5 sm:p-4 rounded-xl text-xs">
                  <div>
                ...
                {inq.quoteAmount && (
                  <div className="text-xs bg-emerald-50 text-emerald-900 p-3 rounded-xl border border-emerald-100 flex items-center justify-between gap-2">
                    <div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs font-bold text-emerald-700 hover:text-emerald-800 px-3 py-2 sm:py-1.5 rounded-lg border border-emerald-200 hover:bg-emerald-50 transition cursor-pointer text-center"
                  >
                    WhatsApp Parent
                  </a>
                  <button
                    ...
                    className="bg-[#0F2E23] hover:bg-[#164E3D] text-[#D4AF37] hover:text-white text-xs font-bold px-4 py-2 sm:py-1.5 rounded-xl transition cursor-pointer shadow-sm text-center"
                  >
              ...
              <form onSubmit={handleSendQuote} className="space-y-4 text-xs">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full mt-1 border rounded-lg p-2.5 font-medium"
                  ></textarea>
                </div>
  ...
  if (activeTab === 'reviews') {
    return (
      <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-100 pb-4 sm:pb-5">
          <div>
            <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">Customer Reviews & Ratings</h2>
            <p className="text-xs text-slate-500 mt-0.5">Verified testimonials from pet parents whose fur babies you safely relocated.</p>
          </div>
          <span className="bg-amber-100 text-amber-900 text-xs font-black px-2.5 py-0.5 rounded-full flex items-center gap-1 shrink-0">
            ...
            >
              ...
              {r.reply ? (
                ...
                <div className="space-y-2 pt-1">
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="text-xs px-3 py-1.5 text-slate-500 cursor-pointer"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => handleSendReviewReply(r.id)}
                      className="text-xs bg-[#0F2E23] text-white font-bold px-3 py-1.5 rounded-lg cursor-pointer"
                    >
                      Post Reply
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="text-xs font-bold text-slate-500 hover:text-[#0F2E23] flex items-center gap-1 cursor-pointer py-1"
                >
  ...
  if (activeTab === 'wallet') {
    return (
      <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 border-b border-slate-100 pb-4 sm:pb-5">
          <div>
            <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">Wallet & Trip Payouts</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="bg-white border border-slate-200 p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-2 shadow-sm">
            ...
            <div className="text-2xl sm:text-3xl font-sans font-black text-[#0F2E23]">₹68,450</div>
          ...
          <div className="bg-white border border-slate-200 p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-2 shadow-sm">
            ...
            <div className="text-2xl sm:text-3xl font-sans font-black text-amber-600">₹9,800</div>
  ...
  if (activeTab === 'profile') {
    return (
      <div className="space-y-5 sm:space-y-6 animate-in fade-in duration-300">
        ...
        <div className="border-b border-slate-100 pb-4 sm:pb-5">
          <h2 className="text-xl sm:text-2xl font-sans font-black text-[#0F2E23]">Transport Agency Profile</h2>
          <p className="text-xs text-slate-500 mt-0.5">Manage your agency licensing, operating hub addresses, and emergency helplines.</p>
        </div>

        <form onSubmit={handleSaveAgencyProfile} className="space-y-5 sm:space-y-6 bg-slate-50/50 p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-slate-200">
          ...
          <div className="flex flex-col sm:flex-row justify-end pt-3 border-t border-slate-200">
            <button
              type="submit"
              className="w-full sm:w-auto bg-[#0F2E23] hover:bg-[#164E3D] text-[#D4AF37] hover:text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition shadow-sm cursor-pointer text-center"
            >
              Save Agency Profile
            </button>
          </div>
